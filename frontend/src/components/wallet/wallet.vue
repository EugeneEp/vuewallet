							...
							<div class="form-block">
								<form class="search-form form-inline">
									...
									<div class="input-group">
								    	...
								    	<input type="date" class="form-control" v-model="sendData.from" value="">
								  	</div>
								  	...
								  	<div class="input-group">
								    	...
								    	<input type="date" class="form-control" v-model="sendData.to" value="">
								  	</div>
								  	<input type="submit" class="btn btn-sm btn-light" value="Поиск">
								  	<a @click='sendData.from="";sendData.to="";openPage(1);'>Сбросить фильтры</a>
								</form>
							</div>
						</div>
					</div>
...
export default {
  ...
  methods: {
  	getTrans(){
  		const path = 'http://127.0.0.1:1337/wallet/transactions/back';
  	...
  	getCsv(){
  		const path = 'http://127.0.0.1:1337/wallet/csv/back';
	    axios.post(path, this.sendData, this.ajaxConfig)
	      .then((res) => {
