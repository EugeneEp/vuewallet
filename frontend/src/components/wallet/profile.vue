...
export default {
	...
	methods: {
		...
		changePhoto(e){
			...
			const path = 'http://127.0.0.1:1337/wallet/profile_picture/back';
			console.log(path);
		...
		userIdentity(e){
			e.preventDefault();
			const path = 'http://127.0.0.1:1337/wallet/profile/back';
			console.log(this.ajaxConfig);
			axios.post(path, this.identity, this.ajaxConfig)
				.then((res) => {
				this.message = res.data.msg;
				this.success = res.data.success;
